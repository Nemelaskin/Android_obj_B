...
public class RoomCrudActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String url = "http://10.0.2.1:5000/api/Companies";
        GetRequest.RequestToGet(url, response -> {
            Response = response.body().string();
            companies = JsonParserArray.fromJsonFromArray(Response, Companies[].class);
            str = new String[companies.length];
            GetNewsData();
        });
    }
    ...
    private void GetNewsData() { // запрос к серверу
        String url = "http://10.0.2.1:5000/api/Rooms";
                ...
                if (response.isSuccessful()) {
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() { // начало генерации круда
                            ...
                            Rooms[] mass = JsonParserArray.fromJsonFromArray(Response, Rooms[].class);
                            ...
                            for (int i = 0; i < mass.length; i++) {
                                ...
                                for(int j = 0; j < companies.length; j++){
                                    str[j] = String.valueOf(companies[j].companyId);
                                    if(companies[j].companyId == mass[i].companyId) checker = j;
                                }
                                ...
                                btn_delete.setOnClickListener(v -> {
                                    String urlForDel = "http://10.0.2.1:5000/api/Rooms/"+ mass[finalI].roomId;
                                    DeleteRequest.RequestToDelete(urlForDel, resp -> {
                                        Intent intent = new Intent(RoomCrudActivity.this, templateActivity.class);
                                        intent.putExtra("ActualUser", actualUserString);
                                        intent.putExtra("From", "RoomCrudActivity");
                                ...
                                btn_edit.setOnClickListener(v -> {
                                    Rooms room = new Rooms(mass[finalI1].roomId, text1.getText().toString(), Integer.parseInt(spinner2.getSelectedItem().toString()), mass[finalI1].coordinatesRoom); // выбрать тип
                                    String json = ObjectParserToJson.fromObjectFromJson(room);
                                    String urlForPut = "http://10.0.2.1:5000/api/Rooms";
                            ...
                            btn_create.setOnClickListener(v ->{
                                ...
                                btn_add.setOnClickListener(s->{
                                    String url = "http://10.0.2.1:5000/api/Rooms";
                                    Rooms room = new Rooms( text1.getText().toString(), Integer.parseInt(spinner2.getSelectedItem().toString())); //выбрать тип
                                    String json = ObjectParserToJson.fromObjectFromJson(room);
    ...
    public void onBackPressed() {
        Intent intent = new Intent(RoomCrudActivity.this, MainAdminActivity.class);
        intent.putExtra("ActualUser", actualUserString);
        startActivity(intent);
    }
}